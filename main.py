...
def LiveRadio():
    req = requests.get('https://noblepeople.co.uk/live-radio/', headers={
                       'User-Agent': 'Chrome', 'Accept-Encoding': 'identity', 'Content-Type': 'text/html'})
...
def LiveTV():
    req = requests.get('https://noblepeople.co.uk/noble-tv/', headers={
                       'User-Agent': 'Chrome', 'Accept-Encoding': 'identity', 'Content-Type': 'text/html'})
    soup = BeautifulSoup(req.text, 'html5lib')
...
def LiveUpComing():
    req = requests.get('https://noblepeople.co.uk/show-schedule/', headers={
                       'User-Agent': 'Chrome', 'Accept-Encoding': 'identity', 'Content-Type': 'text/html'})
    soup = BeautifulSoup(req.text, 'html5lib')
...
def genID():
    mID = random.randint(0000, 9999)

    return mID


def LiveMag():
    req = requests.get('https://noblepeople.co.uk/magazine/', headers={
                       'User-Agent': 'Chrome', 'Accept-Encoding': 'identity', 'Content-Type': 'text/html'})
    soup = BeautifulSoup(req.text, 'html5lib')
    mAllLink = soup.find_all(
...
def LiveNews():
    req = requests.get('https://noblepeople.co.uk/news/', headers={
                       'User-Agent': 'Chrome', 'Accept-Encoding': 'identity', 'Content-Type': 'text/html'})
    soup = BeautifulSoup(req.text, 'html5lib')
